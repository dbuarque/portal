    ...
    switch (e.type) {
        ...
        case 'ACCOUNT_CREDITED':
            switch (e.operation.type) {
                case 'PAYMENT':
                case 'PATH_PAYMENT':
                    return 'Received payment from ' + shortenAddressSpan(e.operation.details.from) + ' for ' +
                ...
                case 'ACCOUNT_MERGE':
                    return 'Credited with ' + assetDetailsToText(e.details.amount, e.details.asset_type, e.details.asset_code) +
                        ' via an account merge from ' + shortenAddressSpan(e.operation.details.account);
                default:
                    return e.type;
            }
        ...
        case 'SIGNER_CREATED':
            return 'New signer ' + shortenAddressSpan(e.details.public_key) + ' added with weight of ' + e.details.weight;
        case 'SIGNER_UPDATED':
            return 'Signer ' + shortenAddressSpan(e.details.public_key) + ' updated with weight of ' + e.details.weight;
        case 'SIGNER_REMOVED':
            return 'Signer ' + shortenAddressSpan(e.details.public_key) + ' removed';
        case 'TRUSTLINE_CREATED':
            return 'New trustline limit of ' + e.details.limit + ' for asset ' + e.details.asset_code + ' issued by ' + shortenAddressSpan(e.details.asset_issuer);
        case 'TRUSTLINE_UPDATED':
            return 'Trustline limit updated to ' + e.details.limit + ' for asset ' + e.details.asset_code + ' issued by ' + shortenAddressSpan(e.details.asset_issuer);
        case 'TRUSTLINE_REMOVED':
            return 'Trustline limit removed for asset ' + e.details.asset_code + ' issued by ' + shortenAddressSpan(e.details.asset_issuer);
        case 'TRADE':
...
function accountDebitedMessage(e) {
    ...
    switch (type) {
        case 'PAYMENT':
            ...
            //    return null;
            //}
            return 'Sent payment to ' + shortenAddressSpan(details.to) + ' for ' +
                assetDetailsToText(details.amount, details.asset_type, details.asset_code);
        case 'PATH_PAYMENT':
            return 'Sent path payment to ' + shortenAddressSpan(details.to) +
                ...
                + ' -> '  + assetDetailsToText(details.amount, details.asset_type, details.asset_code);
        case 'CREATE_ACCOUNT':
            return 'Sent ' + details.starting_balance + ' to create account ' + shortenAddressSpan(details.account);
        default:
            return type;
    }
}
...
function shortenAddressSpan(address) {
    return '<span class="shortened-address" title="' + address + '">' + shortenAddress(address) + '</span>';
}
